...
static PAGE_ALLOCATOR: SpinMutex<PageAllocator> = SpinMutex::new(PageAllocator {
...
impl PageAllocator {
    pub fn alloc_page(&mut self, count: usize) -> Option<NonNull<u8>> {
    ...
    pub unsafe fn free_page(&mut self, ptr: NonNull<u8>) {
        let freed_page_ptr = ptr.cast::<FreePage>();
